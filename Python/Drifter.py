# -*- coding: utf-8 -*-
"""
Created on Mon Mar 25 12:03:20 2024
...
electrode_Dict = {'GC':'Glassy Carbon','Au': 'Gold'}
...
direc = r"C:\Users\Houlberg\Documents\Thesis\Python\Data\GoldElectrode\240322\Gold"
os.chdir(direc)
...
for file in loadedfiles:
    ...
    if "-Activation-" in filename:
        ...
        if "_PEIS_" in filename:
            ...
            cvfiles_post.append(file)  
        caption_post = "Post Activation"
    else:
        if "_PEIS_" in filename:
            peisfiles_pre.append(file)
        elif "-EIS_" in filename:
            peisfiles_pre.append(file)
        elif "_CV_" in filename:
            cvfiles_pre.append(file)
        elif "-CV_" in filename:
            cvfiles_pre.append(file)
            
#%% Load Files and seperate by type. 
#REQUIRES exporting EC-Lab raw binarys (.mpr) as text (.mpt)
direc = r"C:\Users\Houlberg\Documents\Thesis\Python\Data"
os.chdir(direc)
...
#        df['cycle number'] = df['cycle number'].floordiv(2)
    ...
    return df
#%% dummy axes function not working
'''
def _adjust_axes(axes):
...
for file in cvfiles_pre:
    try:
        f = read_mpt(file)
        parsedcv_pre.append(f)
    except:
...
for file in cvfiles_post:
    try:
        ...
        print("Error with file " + str(file))
for file in peisfiles_pre:
    try:
        parsedeis_pre.append(pyi.parse_data(file,file_format=".mpt"))
    except:
        print("Error with file " + str(file))
for file in peisfiles_act:
    try:
        parsedeis_act.append(pyi.parse_data(file,file_format=".mpt"))
    except:
        print("Error with file " + str(file))
for file in peisfiles_post:
    try:
        parsedeis_post.append(pyi.parse_data(file,file_format=".mpt"))
    except:
        print("Error with file " + str(file))
...
fig.suptitle("Gold Electrode Activation")
plt.figure(fig)
...
toplots_eis_drift = parsedeis_post[0]
