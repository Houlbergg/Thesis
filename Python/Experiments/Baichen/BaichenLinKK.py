...
for i, tests in enumerate(explorations):
    ...
    ax.scatter(imp.real, -imp.imag, label='Data', marker=no_fill_markers[0], linewidths=1, color=color_dict["black"], s=10, alpha=1, zorder=2.5)
    imp_fit = fit.get_impedances()
    num_RC = fit.num_RC
    ax.plot(imp_fit.real, -imp_fit.imag, label="#(RC)={}".format(num_RC), marker='', color=color_dict['blue'])
...
for i, tests in enumerate(explorations):
    ...
    num_RC = eis.num_RC
    ax.plot(fit.real, -fit.imag, label="#(RC)={}".format(num_RC), marker='', color=color_dict[i])
