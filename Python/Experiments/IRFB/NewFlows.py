...
fig_directory_nyquist = os.path.join(fig_directory, "Flow")
...
filled_markers = mpl_settings.filled_markers

unit_scale = ""  # If you want to swap to different scale. milli, kilo etc

fig, ax = plt.subplots()
for i, vel in enumerate(chosen):
    imp = vel.get_impedances()
    label = vel_label[i]
...
# plt.legend(loc="best", bbox_to_anchor=(0.0, 0.0, 0.3, 0.7))
plt.legend()
plt.tight_layout()
plt.savefig(
    os.path.join(
        fig_directory_nyquist,
        f"{exp}_FlowRates.png",
    )
)
plt.show()
plt.close()
