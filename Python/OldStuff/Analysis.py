...
freq_mask, Z_mask = fl.get_fZ(first_masked)

inv_hmc = Inverter(basis_freq=freq)
inv_map = Inverter(basis_freq=freq)
inv_hmc_masked = Inverter(basis_freq=freq_mask)
inv_map_masked = Inverter(basis_freq=freq_mask)
...
if not os.path.isfile(os.path.join(pkl_directory, f'inv_hmc_mask.pkl')):
    start = time.time()
    inv_hmc_masked.fit(freq_mask, Z_mask, mode='sample')
    elapsed = time.time() - start
    print('HMC fit time {:.1f} s'.format(elapsed))
    utils.save_pickle(inv_hmc_masked, os.path.join(pkl_directory, f'inv_hmc_mask.pkl'))
else:
    inv_hmc_masked = utils.load_pickle(os.path.join(pkl_directory, f'inv_hmc_mask.pkl'))
...
if not os.path.isfile(os.path.join(pkl_directory, f'inv_map_mask.pkl')):
    start = time.time()
    inv_map_masked.fit(freq_mask, Z_mask, mode='sample')
    elapsed = time.time() - start
    print('MAP fit time {:.1f} s'.format(elapsed))
    utils.save_pickle(inv_map_masked, os.path.join(pkl_directory, f'inv_map_mask.pkl'))
else:
    inv_map_masked = utils.load_pickle(os.path.join(pkl_directory, f'inv_map_mask.pkl'))
